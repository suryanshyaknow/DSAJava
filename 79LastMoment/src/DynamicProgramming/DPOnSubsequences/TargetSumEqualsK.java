...
public class TargetSumEqualsK {
    ...
    public int findTargetSumWaysTabulation(int[] nums, int target) {
        ...
        int sumTotal = S + target;
        if (sumTotal < 0 || sumTotal % 2 != 0) return 0;
        ...
        for (int i=0; i < N; i++) {
            for (int t=0; t <= newTarget; t++) {
                // Edge case
                if (i == 0) { // cuz we gotta cover all the elements
                    if (t == 0 && nums[i] == 0) {
                        dp[i][t] = 2;
                        continue;
                    } else if (t == 0 || nums[i] == t) {
                        dp[i][t] = 1;
                        continue;
                    } else {
                        dp[i][t] = 0;
    ...
    private int helper(int i, int[] nums, int t, int[][] dp) {
        ...
        int notPick = helper(i - 1, nums, t, dp);
        int pick = 0;
        if (nums[i] <= t)
            pick += helper(i - 1, nums, t - nums[i], dp);
